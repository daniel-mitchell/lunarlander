...
double framework<AgentType>::reward() {
  // double reward = -pow(simulator.get_action().thrust, 2)*0.1;
...
template <typename AgentType>
void framework<AgentType>::take_action(mountain_car_simulator::action a) {
...
/*DEBUG*/
    mountain_car_simulator::action a = agent.update(agent_rng, simulator_state(), _reward, terminal);
    if (print_actions) {
...
/*END DEBUG*/
    ...
    if (time_elapsed >= max_time) {
      exit(1); //TODO: Maybe this should throw an error and have the main file exit
    }
  }
...
template <typename AgentType>
double framework<AgentType>::run_episodes(std::mt19937& init_rng, std::mt19937& agent_rng, bool print_actions) {
