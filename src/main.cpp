...
int main (int argc, char* argv[]) {
  ...
  std::vector<int> subspaces { 0, 1, 2 };

  // bool visualize = false;
  // int visualize_from = 0;
  // int visualize_every = 1000;

  enum AgentType {GAUSS=0, BETA=1, MULTIGAUSS=2, MULTIBETA=3, MULTIGAUSS4=4} agentType = GAUSS;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (std::string(argv[i]) == "--agent-steps") agent_time_steps = std::atoi(argv[++i]);
...
//   framework f(mountain_car_simulator(),
...
//               agent_time_steps);
  ...
  switch(agentType) {
    case GAUSS: {
      framework<gauss_agent> f(mountain_car_simulator(init_rng),
              gauss_agent(lambda, lambda_c, alpha_v, alpha_u, alpha_r, initial_value, num_features,
    ...
    case BETA: {
      framework<beta_agent> f(mountain_car_simulator(init_rng),
              beta_agent(lambda, alpha_v, alpha_u, initial_value, num_features,
                                 tile_weight_exponent, trunc_normal, subspaces),
              dt, max_time,
              agent_time_steps);
    ...
    case MULTIBETA: {
      framework<multi_beta_agent> f(mountain_car_simulator(init_rng),
    ...
    case MULTIGAUSS: {
      framework<multi_gauss_agent> f(mountain_car_simulator(init_rng),
              multi_gauss_agent(lambda, lambda_c, alpha_v, alpha_u, alpha_r, epsilon, initial_value, num_features,
    ...
    case MULTIGAUSS4: {
      framework<multi_gauss_4_agent> f(mountain_car_simulator(init_rng),
              multi_gauss_4_agent(lambda, lambda_c, alpha_v, alpha_u, alpha_vv, alpha_uu, alpha_r, epsilon, gamma,
                                    initial_value, num_features, tile_weight_exponent, trunc_normal, subspaces,
                                    weighted_dist_choice, ucb_factor, continuing),
              dt, max_time,
              agent_time_steps);
      double total_return = 0;
      if (continuing) {
