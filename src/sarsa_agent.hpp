#ifndef _SARSA_AGENT_HPP
#define _SARSA_AGENT_HPP
...
class sarsa_agent {
  ...
  double ii = 1;
  double alpha_u, alpha_v, alpha_r;
  VectorXd trace;
  double lambda;
  ...
  cart_pole_simulator::action compute_action([[maybe_unused]] std::mt19937& rng, const VectorXi& features);
...
public:
  ...
  sarsa_agent([[maybe_unused]] double lambda, double alpha_v, double alpha_u, double alpha_r, double epsilon,
                     double gamma,
    ...
    : max_state(6), min_state(6), max_clip_state(6), min_clip_state(6),
      ...
      alpha_u(alpha_u), alpha_v(alpha_v), alpha_r(alpha_r), trace(VectorXd::Zero(tc.get_num_features())),
      lambda(lambda),
