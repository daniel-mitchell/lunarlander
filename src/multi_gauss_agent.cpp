  ...
  struct {
    ...
    unsigned int num_tiles, num_offsets;
      ...
      { // #signed, bounded, tile_size, num_tiles, num_offsets
        // {    false,    true,      0.09,        20,          16 }, //position
        // {     true,    true,     0.007,        20,          16 }  //velocity
...
mountain_car_simulator::action multi_gauss_agent::compute_action(std::mt19937& rng, const VectorXi& features) {
  ...
  return mountain_car_simulator::action(thrust_forward ? thrust_actor_forward.act(rng, features) :\
                                                          thrust_actor_backward.act(rng, features));
}


mountain_car_simulator::action multi_gauss_agent::initialize(std::mt19937& rng, VectorXd state) {
  clip_state(state);
...
mountain_car_simulator::action multi_gauss_agent::update(std::mt19937& rng, VectorXd state,
  ...
  if (learn) {
    ...
    // }
    double td_error = td_critic::evaluate_multi(critic_forward, critic_backward, thrust_forward, features, \
                                                continuing ? reward - rBar : reward, terminal);
