...
public:
  ...
  framework(const cart_pole_simulator& simulator, const AgentType& agent, double dt, double max_time,\
      int agent_time_steps)
  ...
  void take_action(cart_pole_simulator::action a);

  void run_episode(std::mt19937& init_rng, std::mt19937& agent_rng, bool print_actions);
  int run_episodes(std::mt19937& init_rng, std::mt19937& agent_rng, bool print_actions);
