...
mountain_car_simulator::action multi_gauss_4_agent::compute_action([[maybe_unused]] std::mt19937& rng,\
...
/*  // std::cout << 4 << " " << (critic_forward.get_weights() - critic_backward.get_weights()).norm() << std::endl;
  ...
  // return mountain_car_simulator::action(thrust_forward ? 1 : -1);
...
mountain_car_simulator::action multi_gauss_4_agent::initialize(std::mt19937& rng, VectorXd state) {
  clip_state(state);
...
mountain_car_simulator::action multi_gauss_4_agent::update(std::mt19937& rng, VectorXd state,
