ALPHA_U_LIST=(0.01 0.1) #(0.0001 0.0005 0.001 0.005 0.01 0.05 0.1 0.5 1)
ALPHA_V_LIST=(0.01 0.1) #(0.0001 0.0005 0.001 0.005 0.01 0.05 0.1 0.5 1)
ALPHA_UU_LIST=(0.1) #(0.1) #(0.0001 0.0005 0.001 0.005 0.01 0.05 0.1 0.5 1)
ALPHA_VV_LIST=(0.01) #(0.01) #(0.0001 0.0005 0.001 0.005 0.01 0.05 0.1 0.5 1)
ALPHA_R_LIST=(0.0001) #(0.0001 0.0005 0.001 0.005 0.01 0.05 0.1 0.5 1)
...
EPSILON_LIST=(0.05) #(0 0.0001 0.0005 0.001 0.005 0.01 0.05 0.1)
#INAC
#S
        ...
        for LAMBDA_C in ${LAMBDA_C_LIST[@]}; do
            for EPSILON in ${EPSILON_LIST[@]}; do
                for TRUNC_NORMAL in ${TRUNC_NORMAL_LIST[@]}; do
                    TOTAL_RETURN=0;
                    echo -n Running $ALPHA_U $ALPHA_V $ALPHA_UU $ALPHA_VV $ALPHA_R $LAMBDA $LAMBDA_C $EPSILON $TRUNC_NORMAL "   "
                    for I in $(seq -f "%02g" 0 $(expr $NUM_RUNS - 1)); do
                        ...
                        RETURN=$($SCRIPTPATH/build/main --agent-seed $I --init-seed $I --episodes $EPISODES\
                            ...
                            --alpha-uu $ALPHA_UU --alpha-vv $ALPHA_VV --alpha-r $ALPHA_R --lambda $LAMBDA --epsilon $EPSILON\
                            --lambda-c $LAMBDA_C --no-print-episodes --max-time $MAX_TIME);
                        if [ $? -eq 1 ]; then
                            TOTAL_RETURN="Error";
                            break;
                        fi
