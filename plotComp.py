...
for i in range(len(files)):
    ...
    data[i] = np.array([np.mean(dataRaw)+1000000, np.std(dataRaw)/math.sqrt(np.size(dataRaw)), params[paramX], params[paramL]])
    # print(files[i])
    # print(data[i])
...
for line in lines:
    ...
    plt.errorbar(d[:,2],d[:,0],yerr=d[:,1],marker='o',label=nameL+"="+str(line), clip_on=False, zorder=10,elinewidth=2,capsize=5,capthick=2)
    # print(d[:,2])
    # print(d[:,1])
    # print(-d[:,0])
